...
def get_gt(image, frame_id, gt_dict):
    ...
    return (
        ...
        complete,
        team,
    )  # detections are the coordinates of the ball/player, out_scores è quanto è buona la predizione, complete ha dentro tutto
...
def distance_boxes(box1, box2):
    ...
    center_box1 = (
        int((p1_box1[0] + p2_box1[0]) / 2),
        int((p1_box1[1] + p2_box1[1]) / 2),
    )

    p1_box2 = (int(box2[0]), int(box2[1]))
    p2_box2 = (int(box2[0] + box2[2]), int(box2[1] + box2[3]))
    center_box2 = (
        int((p1_box2[0] + p2_box2[0]) / 2),
        int((p1_box2[1] + p2_box2[1]) / 2),
    )

    distance = math.sqrt(
        (center_box1[0] - center_box2[0]) ** 2 + (center_box1[1] - center_box2[1]) ** 2
    )
    return distance
...
def ball_position(line_a, line_b, point):
    ...
    return np.sign(
        (line_b[0] - line_a[0]) * (point[1] - line_a[1])
